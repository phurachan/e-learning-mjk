...
**Last Major Update**: Added Rooms Feature Implementation Example with complete patterns and best practices

## Project Overview
This is a comprehensive **E-Learning Management Platform** built with Nuxt 4 (Vue 3), featuring a full-featured admin panel for managing courses, students, quizzes, and educational content. The project uses TailwindCSS 3.x and DaisyUI 5.1.6 for modern, responsive UI design inspired by AdminLTE.

### Platform Features
- **Student Management**: Complete student enrollment, authentication, and profile management
- **Course Management**: Create and manage courses with multiple lessons and multimedia content
- **Assessment System**: Quiz creation, quiz attempts tracking, and automated grading
- **Room/Class Management**: Organize students into classes/rooms for better organization
- **Reporting & Analytics**: Comprehensive reporting on student progress, course completion, and quiz performance
- **Role-Based Access Control (RBAC)**: Flexible permission system for admins, teachers, and students
- **Multi-Language Support**: Full internationalization with Thai and English languages

## Tech Stack & Setup Issues

### Frontend Stack
- **Framework**: Nuxt 4.1.0 (Vue 3.5.20, SSR enabled)
...
- **Internationalization**: @nuxtjs/i18n 10.1.0 (Thai & English support)
...
- **Password Security**: bcryptjs 3.0.2 for password hashing
- **API Architecture**: Nuxt Server API (H3) with RESTful endpoints (69 endpoints total)
- **File Upload**: Native file handling with FormData support
...
**Additional Components (Not in Categorization Above):**
- **BaseFileUpload** - File upload component with drag & drop support
- **BaseConfirmContainer** - Global confirmation dialog container
- **BaseGlobalAlert** - Global alert system container
- **BaseLanguageSwitcher** - Language/locale switcher component
- **BaseToastContainer** - Toast notification container

**Important Notes:**
- All form components support variants (primary, success, warning, error) and sizes (xs, sm, md, lg)
- **Total: 24 components** (19 categorized + 5 additional containers/utilities)
...
#### Core Admin Models (3 models)
1. **User** (`/server/models/User.ts`)
   - Admin user management with authentication
   - Fields: email, password (hashed), username, firstName, lastName, isActive
   - Relationships: Many-to-Many with Role
   - Methods: Password comparison, password hashing

2. **Role** (`/server/models/Role.ts`)
   - Role-based access control
   - Fields: name, description, isActive
   - Relationships: Many-to-Many with User and Permission

3. **Permission** (`/server/models/Permission.ts`)
   - Granular permission management
   - Fields: name, module, action, description
   - Relationships: Many-to-Many with Role

#### E-Learning Models (6 models)
4. **Student** (`/server/models/Student.ts`)
   - Student profile and authentication
   - Fields: studentId, email, password, firstName, lastName, dateOfBirth, phoneNumber, address, enrollmentDate, isActive
   - Relationships: Many-to-Many with Room, One-to-Many with QuizAttempt
   - Authentication: Separate from admin users, own JWT system

5. **Course** (`/server/models/Course.ts`)
   - Course management and metadata
   - Fields: code, title, description, duration, level, prerequisites, thumbnail, isPublished, publishedAt
   - Relationships: One-to-Many with Lesson
   - Status: Draft or Published

6. **Lesson** (`/server/models/Lesson.ts`)
   - Lesson content within courses
   - Fields: courseId, title, description, content, order, duration, videoUrl, attachments, isPublished
   - Relationships: Belongs to Course, One-to-Many with Quiz
   - Features: Ordering, video support, file attachments

7. **Quiz** (`/server/models/Quiz.ts`)
   - Quiz/assessment creation
   - Fields: lessonId, title, description, timeLimit, passingScore, questions (array), isPublished
   - Relationships: Belongs to Lesson, One-to-Many with QuizAttempt
   - Question Types: Multiple choice, true/false, short answer

8. **QuizAttempt** (`/server/models/QuizAttempt.ts`)
   - Student quiz attempt tracking and grading
   - Fields: studentId, quizId, answers, score, isPassed, startedAt, completedAt, timeSpent
   - Relationships: Belongs to Student and Quiz
   - Grading: Automatic scoring based on quiz answers

9. **Room** (`/server/models/Room.ts`)
   - Class/room organization
   - Fields: code, name, description, academicYear, semester, isActive
   - Relationships: Many-to-Many with Student
   - Purpose: Group students for better organization
...
- `GET /PUT /api/users/[id]/roles` - Get/Update user roles
...
#### Permission Management Endpoints (8 endpoints)
...
- `POST /api/permissions/migrate` - Migrate permissions

#### Student Management Endpoints (7 endpoints)
...
- `POST /api/students/auth/login` - Student authentication
- `GET /api/students/auth/me` - Get current student

#### Course Management Endpoints (5 endpoints)
...
#### Lesson Management Endpoints (7 endpoints)
- `GET /api/lessons` - List lessons (filterable by courseId)
- `POST /api/lessons` - Create new lesson
- `GET /api/lessons/[id]` - Get lesson details
- `PUT /api/lessons/[id]` - Update lesson
- `DELETE /api/lessons/[id]` - Delete lesson
- `PUT /api/lessons/[id]/publish` - Publish/unpublish lesson

#### Quiz Management Endpoints (7 endpoints)
- `GET /api/quizzes` - List quizzes (filterable by lessonId)
...
#### Quiz Attempts Endpoints (6 endpoints)
- `GET /api/quiz-attempts` - List all attempts (admin view)
- `POST /api/quiz-attempts/start` - Start new quiz attempt
- `GET /api/quiz-attempts/[id]` - Get attempt details
- `POST /api/quiz-attempts/[id]/submit` - Submit quiz answers
- `PUT /api/quiz-attempts/[id]/grade` - Manual grading (admin)
- `GET /api/quiz-attempts/my-attempts` - Student's own attempts

#### Room/Class Management Endpoints (5 endpoints)
- `GET /api/rooms` - List all rooms/classes
- `POST /api/rooms` - Create new room
- `GET /api/rooms/[id]` - Get room details with students
- `PUT /api/rooms/[id]` - Update room
- `DELETE /api/rooms/[id]` - Delete room

#### Reporting Endpoints (4 endpoints)
- `GET /api/reports/class/[roomId]` - Class performance report
- `GET /api/reports/course/[courseId]` - Course analytics
- `GET /api/reports/quiz/[quizId]` - Quiz statistics
...
#### Documentation Endpoints (1 endpoint)
- `GET /api/docs` - API documentation

### Pages & Routes (7 Total)

1. **`/index.vue`** - Landing/redirect page
2. **`/login.vue`** - Admin login page (auth layout)
3. **`/admin/index.vue`** - Admin dashboard with statistics
4. **`/admin/components.vue`** - Component showcase/documentation
5. **`/admin/demo.vue`** - CRUD demo page
6. **`/admin/user_management.vue`** - User, Role, Permission management (tabs)
7. **`/admin/i18n-test.vue`** - Internationalization testing page

### Pinia Stores (5 Total)

1. **auth** (`/stores/auth.ts`) - Authentication state, login/logout, user info
2. **users** (`/stores/users.ts`) - User CRUD operations and role assignment
3. **roles** (`/stores/roles.ts`) - Role management
4. **permissions** (`/stores/permissions.ts`) - Permission management
5. **pathMapping** (`/stores/pathMapping.ts`) - Route/breadcrumb mapping configuration

### Feature Modules

#### 1. Student Management Module
- **Purpose**: Manage student enrollment, profiles, and authentication
- **Key Features**:
  - Student CRUD operations with filtering and pagination
  - Bulk import via CSV/Excel
  - Separate authentication system from admin users
  - Student profile management with contact details
  - Room/class assignment
- **Components**: Student list tables, student forms, import dialogs
- **API**: `/api/students/*`

#### 2. Course Management Module
- **Purpose**: Create and organize educational content
- **Key Features**:
  - Course creation with metadata (code, title, description, level)
  - Course publishing workflow (draft → published)
  - Lesson organization within courses with ordering
  - Video integration and file attachments
  - Prerequisites and course duration tracking
- **Components**: Course editors, lesson builders, content managers
- **API**: `/api/courses/*`, `/api/lessons/*`

#### 3. Assessment Module
- **Purpose**: Create quizzes and track student performance
- **Key Features**:
  - Quiz creation with multiple question types
  - Time limits and passing score configuration
  - Automatic grading for objective questions
  - Manual grading support for subjective questions
  - Quiz attempt tracking with scores and completion time
  - Quiz statistics and analytics
- **Components**: Quiz builders, question editors, grading interfaces
- **API**: `/api/quizzes/*`, `/api/quiz-attempts/*`

#### 4. Reporting & Analytics Module
- **Purpose**: Track and visualize learning progress
- **Key Features**:
  - Student progress reports (courses, quizzes, scores)
  - Class/room performance analytics
  - Course completion statistics
  - Quiz performance analysis
  - Export reports to PDF/Excel
- **Components**: Report dashboards, charts, data visualizations
- **API**: `/api/reports/*`

#### 5. Room/Class Organization Module
- **Purpose**: Group students for better management
- **Key Features**:
  - Room creation with academic year and semester
  - Student assignment to multiple rooms
  - Room-based reporting and analytics
  - Bulk student operations within rooms
- **Components**: Room managers, student assignment interfaces
- **API**: `/api/rooms/*`

### Server Utilities & Architecture

#### Query Parser (`/server/utils/queryParser.ts`)
- Advanced query string parsing for filtering, sorting, pagination
- Supports: `filter`, `sort`, `page`, `limit`, `search` parameters
- Integrates with Filter Configurations

#### Filter Configurations (`/server/utils/filter_config/`)
- **room.ts** - Room filtering rules (code, name, academicYear, semester)
- **student.ts** - Student filtering (studentId, name, email, room)
- **lesson.ts** - Lesson filtering (courseId, title, isPublished)
- **course.ts** - Course filtering (code, title, level, isPublished)
- **quiz.ts** - Quiz filtering (lessonId, title, isPublished)
- **userManagement.ts** - User/Role/Permission filtering

#### Response Handler (`/server/utils/responseHandler.ts`)
- Standardized API response format
- Success/error response helpers
- Pagination metadata
- Multilingual error messages (Thai/English)

#### Database Connection (`/server/utils/database.ts`)
...
```
User ←→ Role ←→ Permission (Admin RBAC)

Student ←→ Room (Many-to-Many: Students can be in multiple rooms)
Student → QuizAttempt (One-to-Many: Student has many attempts)

Course → Lesson (One-to-Many: Course has many lessons)
Lesson → Quiz (One-to-Many: Lesson has many quizzes)
Quiz → QuizAttempt (One-to-Many: Quiz has many attempts)
```
...
3. JWT stored in httpOnly cookie (`token`)
4. User data stored in regular cookie (`user`)
5. All admin API requests include Bearer token automatically
6. Token expires after 7 days

#### Student Authentication
1. Student logs in via `/api/students/auth/login`
2. Separate JWT system from admin
3. Student session stored in cookies
4. Student can access their own courses, quizzes, and attempts
5. Limited API access compared to admin

## Rooms Feature Implementation Example

This section documents the complete implementation of the Rooms management feature, serving as a reference pattern for future E-Learning features (Students, Courses, Lessons, Quizzes).
...
composables/
├── data_models/
│   └── rooms.ts              # Room entity interface
├── store_models/
│   └── rooms.ts              # Pinia store state & request interfaces
└── constants/
    └── api.ts                # API endpoint constants (updated)

stores/
└── rooms.ts                  # Pinia store with CRUD actions

pages/admin/
└── rooms.vue                 # Card Grid UI with filters
...
**File**: `composables/data_models/rooms.ts`
...
**Purpose**: Define the core entity structure matching the backend model.

### 2. Store Model Definition

**File**: `composables/store_models/rooms.ts`
...
**Key Pattern**: Import Room from data_models to avoid duplication. Store models define requests/state, data models define entities.

### 3. API Endpoint Constants

**File**: `composables/constants/api.ts` (add to existing file)
...
**Pattern**: Centralize all API endpoints for easy maintenance and refactoring.
...
**Pattern**: Follow BaseRequestData pattern with loadingState, successState, errorState. All actions use try-catch with proper error handling.

### 5. Page Implementation (Card Grid UI)

**File**: `pages/admin/rooms.vue`

#### Key Features Implemented:
- **Card Grid Layout**: 1-4 columns responsive grid
- **Search**: Debounced search (500ms) with magnifying glass icon
- **Filters**: Grade, Academic Year, Active Status using BaseSelect
- **Modal**: Create/Edit form with validation
- **Dropdown Menu**: Edit/Delete actions with proper event handling
- **Pagination**: BasePagination with page size control
- **i18n**: Full Thai/English translation support
- **Loading States**: BaseLoading component during API calls

#### Critical Implementation Patterns:

##### Query Parameter Format (IMPORTANT!)
```typescript
const query: any = {
  pagination: {
...
// When filter values exist, add to nested filter object
if (filters.value.grade) {
  query.filter.grade = parseInt(filters.value.grade)
}

if (filters.value.academicYear) {
  query.filter.academicYear = parseInt(filters.value.academicYear)
}
...
**API sends nested objects** like: `{ pagination: { page: 1, limit: 10 }, filter: { grade: 2 } }`

**Query string conversion happens in httpClient**, resulting in: `?pagination[page]=1&pagination[limit]=10&filter[grade]=2`

##### Icon Naming Convention (IMPORTANT!)
```vue
<!-- CORRECT: kebab-case -->
<BaseIcon name="plus" />
<BaseIcon name="magnifying-glass" />
<BaseIcon name="ellipsis-vertical" />
<BaseIcon name="pencil" />
<BaseIcon name="trash" />
<BaseIcon name="academic-cap" />

<!-- WRONG: PascalCase (will not display) -->
<BaseIcon name="PlusIcon" />
<BaseIcon name="MagnifyingGlassIcon" />
```

**Rule**: All Heroicons must use kebab-case naming, not PascalCase.

##### Modal Integration Pattern
```vue
<!-- Use :visible prop, NOT v-model -->
<BaseModal
  :visible="isModalOpen"
  @update:visible="isModalOpen = $event"
  :title="isEditing ? $t('rooms.edit') : $t('rooms.create')"
>
  <!-- Modal content -->
</BaseModal>
```

**Rule**: BaseModal uses `:visible` prop with `@update:visible` event, not v-model.

##### Event Bubbling Prevention
```vue
<div class="card" @click="viewRoom(room)">
  <!-- Card content that should trigger viewRoom -->

  <div class="dropdown" @click.stop>
    <!-- Dropdown that should NOT trigger viewRoom -->
    <button>Actions</button>
  </div>
</div>
```

**PROBLEM**: Without `@click.stop`, clicking dropdown button triggers both dropdown AND card click.

**SOLUTION**:
1. Remove `@click` from card wrapper
2. Add `@click.stop` to dropdown container
3. Add `@click="viewRoom(room)"` to specific clickable areas (title, content, badge)

##### Confirmation Dialog Pattern
```typescript
// WRONG: useConfirm directly
const confirm = useConfirm()
await confirm.show()  // TypeError: show is not a function

// CORRECT: Use toast.confirm() wrapper
const toast = useToast()
const confirmed = await toast.confirm(
  title,
  message,
  'error'  // variant: primary, error, warning, success
)
...
**Rule**: Always use `toast.confirm()` which internally handles useConfirm properly.
...
**Rule**: Always debounce search input to avoid excessive API calls. Reset page to 1 on search.
...
<BaseSelect
  ...
  @change="handleFilter"  // Triggers when value changes
/>
```

**Rule**: BaseSelect emits `@change` event with `(value, option)` parameters. Always reset page to 1 when filters change.

### 6. Internationalization

**Files**: `i18n/locales/th.json` and `en.json`
...
```json
{
  ...
  "rooms": {
    ...
    "subtitle": "จัดการข้อมูลห้องเรียนทั้งหมด",
    "create": "สร้างห้องเรียน",
    "edit": "แก้ไขห้องเรียน",
    "form": {
      "name": "ชื่อห้องเรียน",
      "code": "รหัสห้องเรียน",
    ...
    "createSuccess": "สร้างห้องเรียนสำเร็จ",
    "updateSuccess": "แก้ไขห้องเรียนสำเร็จ",
    "deleteSuccess": "ลบห้องเรียนสำเร็จ",
    "deleteConfirm": "ยืนยันการลบ",
    "deleteConfirmMessage": "คุณต้องการลบห้องเรียน {name} ใช่หรือไม่?"
  }
}
```

**Pattern**: Organize translations by feature module. Use interpolation `{name}` for dynamic values.

### Common Errors & Solutions

#### Error 1: Modal Not Showing
**Symptom**: Clicking "Create" button does nothing.
**Cause**: Using `v-model="isModalOpen"` instead of `:visible`.
**Solution**: Change to `:visible="isModalOpen" @update:visible="isModalOpen = $event"`.

#### Error 2: Icons Not Visible
**Symptom**: Icons appear as empty space in both light and dark mode.
**Cause**: Using PascalCase icon names (e.g., `PencilIcon`).
**Solution**: Change to kebab-case (e.g., `pencil`).

#### Error 3: Event Bubbling
**Symptom**: Clicking dropdown icon opens both dropdown menu and edit modal.
**Cause**: Click event propagates from dropdown button to card container.
**Solution**: Add `@click.stop` to dropdown container div.

#### Error 4: TypeError - confirm.show is not a function
**Symptom**: Error when trying to show confirmation dialog.
**Cause**: Using `const confirm = useConfirm()` and calling `confirm.show()`.
**Solution**: Use `toast.confirm()` method instead.

#### Error 5: Filters Not Working
**Symptom**: API called but results not filtered.
**Cause**: Wrong query parameter format (flat object instead of nested).
**Solution**: Use nested object format with `pagination` and `filter` objects (see Query Parameter Format above).

### Best Practices for Future Features

1. **Start with Data Models**: Define entity interfaces first in `data_models/`
2. **Define Store Models**: Create request/response interfaces in `store_models/`
3. **Add API Constants**: Centralize endpoints in `constants/api.ts`
4. **Implement Store**: Follow BaseRequestData pattern with proper error handling
5. **Build UI**: Use Card Grid for browseable data, DataTable for detailed listings
6. **Add Translations**: Complete i18n for both Thai and English
7. **Test Thoroughly**: Check modal, icons, events, filters, pagination
8. **Document Patterns**: Update CLAUDE_KNOWLEDGE.md with any new patterns learned

### Performance Considerations

- **Debounced Search**: 500ms delay prevents excessive API calls during typing
- **Pagination**: Limits results to 12 items per page (configurable)
- **Lazy Loading**: Only loads current page data, not entire dataset
- **Optimistic Updates**: UI updates immediately while API call is in flight
- **Error Recovery**: Proper error handling with user-friendly toast messages

### UI/UX Decisions

- **Card Grid vs DataTable**: Cards chosen for Rooms/Courses (visual browsing), DataTable for Students (detailed data)
- **Filter Placement**: Filters positioned horizontally above content for easy access
- **Modal Size**: Large (`lg`) modal for forms with multiple fields
- **Action Menu**: Dropdown menu for edit/delete keeps UI clean
- **Status Badge**: Color-coded badges (success/error) for active/inactive status
- **Empty State**: Helpful message with "Create First" button when no data

### Testing Checklist for New Features

- [ ] Modal opens and closes correctly
- [ ] Icons display in light and dark mode
- [ ] Dropdown menu doesn't trigger card click
- [ ] Search works with debounce
- [ ] Filters update results correctly
- [ ] Pagination changes pages
- [ ] Create form saves new records
- [ ] Edit form updates existing records
- [ ] Delete shows confirmation and removes record
...
The project uses **nested object format** for API queries. The httpClient automatically converts nested objects to query string parameters.
...
// httpClient converts to: ?pagination[page]=1&pagination[limit]=10&filter[grade]=2&filter[isActive]=true&search=keyword
```

```typescript
// WRONG: Flat format (will not filter properly)
const query = {
  page: 1,
  limit: 10,
  grade: 2,
...
**Rule**: Always use nested `pagination` and `filter` objects when calling store actions.

#### Icon Naming Convention
All Heroicons must use **kebab-case**, not PascalCase.
...
#### Modal Integration
BaseModal uses `:visible` prop pattern, not v-model.
...
Always use `toast.confirm()`, not `useConfirm()` directly.

```typescript
// CORRECT
const toast = useToast()
const confirmed = await toast.confirm(title, message, 'error')
...
This documentation was significantly updated to reflect the evolution from a simple admin panel to a comprehensive **E-Learning Management Platform**.

### **Key Changes Made**

#### 1. **Project Overview**
- ✅ **Updated**: From "admin panel" to "E-Learning Management Platform"
- ✅ **Added**: Platform features section highlighting all major capabilities
- ✅ **Added**: Multi-language support mention

#### 2. **Tech Stack**
- ✅ **Split**: Frontend and Backend stack sections
- ✅ **Updated**: Package versions (Nuxt 4.1.0, Vue 3.5.20)
- ✅ **Added**: Backend technologies (MongoDB 8.18.0, JWT, bcryptjs)
- ✅ **Added**: Internationalization (@nuxtjs/i18n 10.1.0)
- ✅ **Added**: File upload support mention

#### 3. **E-Learning Platform Architecture** (NEW SECTION)
- ✅ **Added**: Complete database models documentation (9 models)
  - Core Admin Models: User, Role, Permission
  - E-Learning Models: Student, Course, Lesson, Quiz, QuizAttempt, Room
- ✅ **Added**: Complete API endpoints documentation (69 endpoints)
  - Categorized by domain (Auth, Users, Roles, Permissions, Students, Courses, Lessons, Quizzes, Quiz Attempts, Rooms, Reports, Files)
- ✅ **Added**: Pages & Routes documentation (7 pages)
- ✅ **Added**: Pinia Stores documentation (5 stores)
- ✅ **Added**: Feature Modules documentation
  - Student Management Module
  - Course Management Module
  - Assessment Module
  - Reporting & Analytics Module
  - Room/Class Organization Module
- ✅ **Added**: Server Utilities & Architecture
  - Query Parser
  - Filter Configurations
  - Response Handler
  - Database Connection
  - JWT Utilities
- ✅ **Added**: Data Relationships diagram
- ✅ **Added**: Authentication Flow (Admin & Student)

#### 4. **Component Documentation**
- ✅ **Updated**: Total components from 19 to **24 components**
- ✅ **Added**: 5 new components documentation
  - BaseFileUpload
...
**Before Update:**
- Database Models: 3 documented
- API Endpoints: ~15 documented
- Components: 19 documented
- Pages: 4 documented
- Feature Modules: None documented

**After Update:**
- Database Models: **9 documented** (+6)
- API Endpoints: **69 documented** (+54)
- Components: **24 documented** (+5)
- Pages: **7 documented** (+3)
- Feature Modules: **5 documented** (NEW)
- Server Utilities: **6 documented** (NEW)
...
- ✅ Filter configurations documented
- ✅ Best practices established
...
- ✅ API endpoint counts confirmed
- ✅ Component counts audited
- ✅ Technology stack verified

### **Files Affected**

**Primary File:**
- `/CLAUDE_KNOWLEDGE.md` - Main documentation file (2,354 → 2,700+ lines)

**Backup Created:**
- `/CLAUDE_KNOWLEDGE.md.backup` - Original version before update

### **Update Methodology**

1. **Audit Phase**: Comprehensive codebase analysis using Explore agent
2. **Verification Phase**: File and endpoint counting validation
3. **Comparison Phase**: Documentation vs reality gap analysis
4. **Update Phase**: Systematic section-by-section updates
5. **Validation Phase**: Cross-checking updated content

### **Impact on Development**

#### For AI Assistants
- ✅ Complete project understanding
- ✅ Accurate component references
- ✅ Correct API endpoint knowledge
- ✅ Proper architecture awareness
- ✅ Better code generation context
...
1. **After Adding Features**: Update corresponding Feature Module section
2. **After Creating APIs**: Add endpoints to API Endpoints section
3. **After Adding Components**: Update Components count and list
4. **After Schema Changes**: Update Database Models section
5. **After Adding Pages**: Update Pages & Routes section

### **Version Information**

- **Documentation Version**: 2.0 (October 2025)
- **Previous Version**: 1.0 (September 2025)
- **Update Type**: Major - Reflects complete platform evolution
- **Breaking Changes**: None (documentation only)

---

**Documentation Update Status**: ✅ COMPLETE - CLAUDE_KNOWLEDGE.md now accurately reflects the full E-Learning Management Platform architecture with comprehensive coverage of all 9 database models, 69 API endpoints, 24 components, 7 pages, 5 feature modules, and complete server-side architecture.
